...
impl DiskManager {
    ...
    pub fn new(path: &Path) -> Result<Self, std::io::Error> {
        ...
        Ok(Self { file: file, num_pages: num_pages })
    }
    ...
    /// 
    pub fn read_page(&mut self, id: &PageId, buffer: &mut [u8; PAGE_SIZE]) -> Result<(), std::io::Error> {
    ...
    /// * The underlying file I/O operation fails.
    /// 
    pub fn write_page(&mut self, id: &PageId, buffer: &[u8; PAGE_SIZE]) -> Result<(), std::io::Error> {
        let offset = id.value() * PAGE_SIZE as u64;
        self.file.seek(SeekFrom::Start(offset))?;
